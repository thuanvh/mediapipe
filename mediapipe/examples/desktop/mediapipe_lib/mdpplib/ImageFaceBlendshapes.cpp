...
using FaceLandmarks = std::vector<::mediapipe::NormalizedLandmarkList>;
     ...
     */
    absl::StatusOr<FaceLandmarks> ImageFaceBlendshapes::Run(const uint8_t* pixel_data, int width, int height, int64_t timestamp_us) {
        ...
        // Copy pixel data to ImageFrame
        // ImageFrame usually has alignment, so we need to copy line by line or use CopyFromBuffer if available
        // But for SRGB, width * 3 is the stride if no extra padding.
        // mp_frame->PixelData() returns the internal buffer.
