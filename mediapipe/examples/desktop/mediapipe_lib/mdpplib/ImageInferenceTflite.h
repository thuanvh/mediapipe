...
public:
  ...
  std::vector<float> Inference(const uint8_t* pixel_data, int width, int height, bool use_scale = true, float scale = 1 / 255.0f, float subtract = 0.0f);
  int Predict(const uint8_t* pixel_data, int width, int height, bool use_scale, float scale, float subtract, std::vector<float>& prob);
  void DumpModel();
