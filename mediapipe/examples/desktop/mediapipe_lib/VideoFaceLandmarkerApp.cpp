...
int main(int argc, char** argv) {
    ...
    const std::string kGraphPath = "mediapipe/graphs/face_mesh/face_mesh_desktop_live.pbtxt";
    
    mdpplib::VideoFaceLandmarker landmarker;
    ...
    // Example of running the loop (e.g., from a webcam)
    ...
    while (cap.read(frame)) {
        int64_t timestamp_us = frame_count * 1000 * 1000 / 30; // Mock 30 FPS timestamp

        // Run detection
        absl::StatusOr<mdpplib::FaceLandmarks> result = landmarker.Run(frame, timestamp_us);
